...
public:
    ...
    void ajouterEvenement(const string& date, const string& description) {
        journal[date] = description;
        cout << "evenement ajoute le " << date << " : " << description << endl;
    }

    void modifierEvenement(const string& date, const string& nouvelleDescription) {
        auto it = journal.find(date);
        if (it != journal.end()) {
            journal[date] = nouvelleDescription;
            cout << "evenement du " << date << " modifie : " << nouvelleDescription << endl;
        } else {
            cout << "Aucun evenement trouve à la date " << date << endl;
        }
    }

    void supprimerEvenement(const string& date) {
        auto it = journal.find(date);
        if (it != journal.end()) {
            journal.erase(it);
            cout << "evenement du " << date << " supprime" << endl;
        } else {
            cout << "Aucun evenement trouve à la date " << date << endl;
        }
    }
...
int main() {
    ...
    p1->ajouterEvenement("2024-01-01", "Rencontre avec un autre pingouin");
    p1->ajouterEvenement("2024-01-02", "Decouverte d'un nouveau lieu de peche");

    p1->modifierEvenement("2024-01-01", "Rencontre avec un groupe de pingouins");

    p1->afficherJournal();

    p1->supprimerEvenement("2024-01-02");
